...
\fi
  ...
  \begin{frame}
    ...
    \cite{Horowitz}\cite{TorqueBoards}\cite{CDV}%\cite{STNEN17128}
  \end{frame}
...
% ----------------------------------------------
% Zadanie práce / Assignment
% ----------------------------------------------

  % \begin{frame}
  %   \frametitle{Zadanie práce}
  %   \begin{enumerate}
  %     \item Shrňte požadavky kladené na výrobek typu longboard a případy jeho užití; obzvláště se zaměřte na výrobky typu longboard s elektrickým pohonem (LEP).
  %     \item Vytvořte přehled v oblasti přístupů k vývoji výrobků typu LEP, proveďte jejich rozbor a shrňte jejich výhody a nevýhody.
  %     \item Na základě výstupu bodů 1 a 2 zvolte technické a programové prostředky vhodné pro konstrukci výrobku typu LEP, jeho ovládání a monitorování. S využitím zvolených prostředků navrhněte architekturu a princip činnosti výrobku typu LEP a způsob ověření jeho vlastností.
  %     \item Návrh z bodu 3 realizujte formou protypu výrobku typu LEP, průběh realizace zdokumentujte a výsledky dílčích realizačních etap vhodně ověřte.
  %     \item Vlastnosti výsledné realizace vhodně demonstrujte a ověřte, přinejmenším s ohledem na požadavky a případy užití z bodu 1 zadání.
  %     \item Zhodnoťte vlastnosti výsledné realizace; identifikujte její silné a slabé stránky, navrhněte možné směry jejího vylepšení a rozveďte ty, které považujete za nejvíce perspektivní.
  %   \end{enumerate}
  % \end{frame}