...
\documentclass[]{template/fitthesispresn}
...
%\documentclass[english]{template/fitthesispresn}
...
%\documentclass[wideratio]{template/fitthesispresn}
...
\projectinfo{
  ...
  title={Longboard \\s~elektrickým pohonem}, % Název prezentace / Presentation title (The whole title of the presented work suitably divided into lines that are optically balanced)
  title.footer={Longboard s~elektrickým pohonem}, % Název prezentace - text zobrazovaný vedle čísla slajdu / Presentation title - displayed next to the slide number (The full title of the presented work, it may be suitably abbreviated to fit the footer)
  ...
  supervisor.name={Josef}, % Jméno vedoucího / Supervisor name
  supervisor.surname={Strnadel}, % Příjmení vedoucího / Supervisor surname
...
\begin{document}
    ...
    \ifczech
        \input{projekt-presentation-slides}  % Slajdy v češtině / Slides in Czech
    \else
        \input{projekt-presentation-slides_en}  % Slajdy v angličtině / Slides in English
    \fi
\end{document}
